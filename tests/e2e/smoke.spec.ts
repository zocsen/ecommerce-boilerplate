...
test.describe("Home page", () => {
  test("loads and displays store branding + hero section", async ({ page }) => {
    ...
    await expect(
      page.locator("h1", { hasText: "Prémium minőség" }),
    ).toBeVisible()
...
test.describe("Products page", () => {
  test("renders with heading and no crash", async ({ page }) => {
    ...
    await expect(
      page.locator("h1", { hasText: "Termékek" }),
    ).toBeVisible()
...
test.describe("Cart page", () => {
  test("shows empty cart message when no items", async ({ page }) => {
    ...
    // The actual text in the codebase is "A kosarad ures" (no accents)
    await expect(
      page.getByText(/kosarad\s+(ü|u)res/i),
    ).toBeVisible()
  })
})
...
test.describe("Login page", () => {
  test("displays login form with email, password, and submit", async ({
    page,
  }) => {
    ...
    await expect(
      page.locator("h1", { hasText: "Bejelentkezes" }),
    ).toBeVisible()
...
test.describe("Register page", () => {
  test("displays registration form with required fields", async ({
    page,
  }) => {
    ...
    await expect(
      page.locator("h1", { hasText: "Regisztracio" }),
    ).toBeVisible()
...
test.describe("Legal pages", () => {
  ...
  test("privacy page has content", async ({ page }) => {
    ...
    await expect(
      page.locator("h1", { hasText: "Adatvédelmi Tájékoztató" }),
    ).toBeVisible()
  ...
  test("shipping and returns page has content", async ({ page }) => {
    ...
    await expect(
      page.locator("h1", { hasText: "Szállítás és Visszaküldés" }),
    ).toBeVisible()
...
test.describe("Navigation", () => {
  test('clicking "Termékek" nav link navigates to /products', async ({
    page,
  }) => {
    await page.goto("/")
