...
19. [Feature Roadmap](#feature-roadmap)
    - [P0 — Blocks Production (8 features)](#p0--blocks-production-8-features)
    - [P1 — First Client Launch (11 features)](#p1--first-client-launch-11-features)
    - [P2 — Growth Phase (13 features)](#p2--growth-phase-13-features)
    - [P3 — Premium Differentiators (13 features)](#p3--premium-differentiators-13-features)
...
2. **Flag-based:** Add `is_agency_owner boolean DEFAULT false` to profiles. Simpler migration, reuses existing `admin` role for RLS, checks the flag in middleware and security helpers.

Recommended: Option 2 (flag-based) for minimal disruption to existing RLS policies. The middleware would check `role === 'admin' && is_agency_owner` for agency-level routes.
...
- **CTA per plan:** "Kerd ajanlatunkat" (Request a quote) button for each plan that is not the current one. Links to a contact form or sends a request to the agency.
- **Access:** All admin roles can view.

### `/admin/agency/clients` (Agency Client Management) **(PLANNED)**

**Purpose:** Super-admin view for the agency to manage all client shops, their subscriptions, billing, and feature overrides. Only visible to users with the `agency_admin` flag. NOT visible to regular shop admins or agency viewers.
...
- **Click row => Client detail panel** (or side drawer):
  - **Subscription management:** Change plan, switch billing cycle (monthly/annual), set custom pricing (override base plan price for this specific client), toggle annual discount
  ...
  - **Invoice actions:** "Szamla keszitese" (Generate invoice) button — creates an invoice for the current billing period using the existing Billingo/Szamlazz adapter. Same UX pattern as order invoicing on `/admin/orders/[id]`.
...
- "Nyomtat├ís" (Print) icon button in the order detail header.
...
- Content:
  ...
  - "Szallitolevel" (Packing slip) heading.
...
- "Nyomtat├ís" button in bulk action toolbar.
